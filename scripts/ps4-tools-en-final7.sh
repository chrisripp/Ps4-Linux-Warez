...
#========================================================================
# ps4-tools.sh — For Debian, Ubuntu, Mint .....
...
# PS4-TOOLS — persistent configuration
...
<small>PS4-TOOLS v6.0 — $(date '+%d/%m/%Y %H:%M')</small>\n" \
        \
        --field="":LBL "" \
...
AIDE_LABELS=(
    "PS4-TOOLS: Préférences"      # button 1
    "PS4-TOOLS: Full USB/SSD"             # button 2
    "PS4-TOOLS: Connexion FTP" # button 3
    "PS4-TOOLS: Connexion SSH" # button 4
    "PS4-TOOLS: Nmap"             # button 5
    "Update All Triki1 Distributions"             # button 6
    "Other method SSD/USB multi boot"             # button 7
...
AIDE_PATHS=(
    "$HOME/PROJECT-PS4/tutorials/PREFERENCE PS4-TOOLS.pdf"   # 1
    "$HOME/PROJECT-PS4/tutorials/PS4-TOOLS AIO SSDUSB PS4LINUX.pdf"   # 2
    "$HOME/PROJECT-PS4/tutorials/PS4-TOOLS CONNEXION FTP.pdf"   # 3
    "$HOME/PROJECT-PS4/tutorials/CONNEXION SSH.pdf"   # 4
    "$HOME/PROJECT-PS4/tutorials/Trouver ladresse IP de votre.pdf"   # 5
    "$HOME/PROJECT-PS4/tutorials/Mettre a jour Les Distributions TRIKI1.pdf"   # 6
    "$HOME/PROJECT-PS4/tutorials/Create a Multiboot SSD2.pdf"   # 7
    "$HOME/PROJECT-PS4/tutorials/MOUNT-hdd-intern-ps4.pdf"   # 8
    "$HOME/PROJECT-PS4/tutorials/MOUNT-hdd-intern-ps4.pdf"   # 9
    "$HOME/PROJECT-PS4/tutorials/MOUNT-hdd-intern-ps4.pdf"   # 10
)
# ──────────────────────────────────────────────────────────────────────
...
do_prepare_usb() {
    ...
    #    node when physically connected via USB.
    local usb_disks=()
    ...
    # We select the WHOLE DISK (not a partition): it will be fully
    # repartitioned.
    ...
    for d in "${usb_disks[@]}"; do
        ...
        usb_list+=("/dev/$d" "${dsize}  |  ${dmodel:-USB device}")
    done

    local sel_dev
    sel_dev=$(yad --center --borders=10 \
        --title="Select USB disk" \
        --list \
        --text="<b>Prepare PS4 boot USB drive</b>\n\nSelect the target USB <b>disk</b>:\n⚠️  <b>ALL data on the disk will be ERASED</b> (the disk will be fully repartitioned)." \
        --column="Disk" \
        --column="Size  |  Model" \
        "${usb_list[@]}" \
        --print-column=1 --separator="" \
    ...
    # Total disk size, to compute the maximum possible size for the ext4
    # partition (disk - 130 MiB for FAT32 - small margin).
    ...
    # Search for default bzImage / initramfs in the project
    ...
    # Step 1 form: FAT32 label + number of ext4 partitions
    local step1
    step1=$(yad --center --borders=10 \
        --title="Partition the USB disk" \
        --form \
        --text="<b>Partitioning $sel_dev</b>\n\n<span foreground='red'><b>⚠️  ALL data on $sel_dev will be destroyed.</b></span>\n\nA 130 MB <b>FAT32</b> partition will be created first (receives bzImage / initramfs / bootargs / vram), followed by one or more <b>ext4</b> partitions.\n" \
        --field="FAT32 partition label:":TEXT           "PS4BOOT" \
        --field="Number of ext4 partitions (1 to 4):":NUM "1!1..4!1" \
    ...
    # Step 2 form: size + label for each ext4 partition
    local default_size=$(( max_gb / nb_ext4 ))
    [ "$default_size" -lt 1 ] && default_size=1
    local ext4_fields=() i
    for ((i = 1; i <= nb_ext4; i++)); do
        ext4_fields+=(--field="ext4 partition #$i size (GB):":NUM "${default_size}!1..${max_gb}!1")
        ext4_fields+=(--field="ext4 partition #$i label:":TEXT "PS4ROOT$([ "$i" -gt 1 ] && echo "$i")")
    done
    ext4_fields+=(--field="Last ext4 partition uses all remaining space:":CHK "TRUE")

    local step2
    step2=$(yad --center --borders=10 \
        --title="ext4 partitions" \
        --form \
        --text="<b>ext4 partitions configuration</b>\n\nSpace available after FAT32 (130 MB): about ${max_gb} GB.\n<small>If the checkbox below is checked, the size entered for the last partition is ignored: it will use all remaining space.</small>\n" \
        "${ext4_fields[@]}" \
        --button="Cancel:1" --button="Next:0" \
        --width=680)
    [ $? -ne 0 ] || [ -z "$step2" ] && return
    ...
    # Validate the total requested size
    ...
    if [ "$sum" -gt "$max_gb" ]; then
        yad_err "The total requested size (${sum} GB) exceeds the available space (${max_gb} GB).\nStart over with smaller sizes."
        return
    fi
    ...
    out=$(yad --center --borders=10 \
        --title="Boot files" \
        --form \
        --text="<b>Files copied to the root of the FAT32 partition \"$lbl_fat\"</b>\n" \
    ...
    if [ "$confirm_erase" != "TRUE" ]; then
        yad_err "You must check the confirmation box to erase $sel_dev."
        return
    fi
    ...
    local plan_txt=" • FAT32 \"$lbl_fat\" — 130 MB\n"
    for ((i = 0; i < nb_ext4; i++)); do
        if [ "$autofill_last" = "TRUE" ] && [ "$i" -eq "$last_idx" ]; then
            plan_txt+=" • ext4  \"${ext4_labels[$i]}\" — all remaining space\n"
        else
            plan_txt+=" • ext4  \"${ext4_labels[$i]}\" — ${ext4_sizes[$i]} GB\n"
        fi
    done

    # Final explicit confirmation (red button)
    yad --center --borders=10 --title="⚠️ Final confirmation" \
        --text="<span foreground='red'><b>FINAL CONFIRMATION</b></span>\n\nDisk <b>$sel_dev</b> will be <b>completely erased and repartitioned</b>:\n\n${plan_txt}\nThis action is <b>irreversible</b>.\n" \
        --button="Cancel:1" --button="Yes, erase and partition:0" \
        --width=520
    [ $? -ne 0 ] && return

    # Partition names: sdX1/sdX2/... normally, but mmcblk0p1/nvme0n1p1 if
    # the disk name ends with a digit.
    partdev() {
        if [[ "$sel_dev" =~ [0-9]$ ]]; then
            echo "${sel_dev}p$1"
        else
    ...
    # Build parted commands (compute MiB offsets)
...
"
        mkfs_cmds+="echo '--- Formatting ext4 #$((i+1)) (${pext[$i]}) ---'
...
"
    ...
    for ((i = 0; i < nb_ext4; i++)); do
        if [ "$autofill_last" = "TRUE" ] && [ "$i" -eq "$last_idx" ]; then
            summary_txt+="  Partition $((i+2)) (ext4,  ${pext[$i]}) : ${ext4_labels[$i]} (remaining disk space)\n"
        else
            summary_txt+="  Partition $((i+2)) (ext4,  ${pext[$i]}) : ${ext4_labels[$i]} (${ext4_sizes[$i]} GB)\n"
        fi
    done
...
echo ''
echo '✓ USB drive ready — you can safely remove it.'
echo -e "$summary_txt"
read -rp '[Press Enter to close]'
UEOF
    chmod +x "$tmpscript"
    case "$TERM_BIN" in
        xfce4-terminal) xfce4-terminal --title="Prepare PS4 boot USB drive" -e "bash -c '$tmpscript; rm -f $tmpscript'" ;;
        gnome-terminal) gnome-terminal --title="Prepare PS4 boot USB drive" -- bash -c "$tmpscript; rm -f $tmpscript" ;;
        mate-terminal)  mate-terminal  --title="Prepare PS4 boot USB drive" -e "bash -c '$tmpscript; rm -f $tmpscript'" ;;
        *)              xterm -title "Prepare PS4 boot USB drive" -e bash -c "$tmpscript; rm -f $tmpscript" ;;
    esac
}
...
PYEOF
    ...
    while true; do
        sel=$(yad --center --borders=10 \
            ...
            --list \
            --text="$(printf '<big><b><span foreground="#4FC3F7">⬇  PS4 Linux Downloads</span></b></big>\n'
                    '<small>Source : dionkill.github.io  —  <b>%s entries</b></small>\n\n'
                    '<b>🐧 Kernels   →</b> <tt>%s</tt>\n'
                    '<b>💾 Initramfs →</b> <tt>%s</tt>\n'
                    '<b>📦 Distros   →</b> <tt>%s</tt>\n\n'
                    '<small>Select → <b>⬇ Download</b> (wget -c) or <b>🌐 Browser</b></small>'
                    "$nb_entries" \
                    "${DL_KERNELS_DIR//&/&amp;}" \
                    "${DL_INITRAMFS_DIR//&/&amp;}" \
                    "${DL_DISTROS_DIR//&/&amp;}")" \
...
do_open_url_payload_launcher() {
    xdg-open "https://github.com/DEBARSHI-SARKAR/Payload-launcher" >/dev/null 2>&1 &
}
export -f do_open_url_payload_launcher

do_dl_payload_launcher() {
    xdg-open "https://github.com/DEBARSHI-SARKAR/Payload-launcher/releases/tag/v2.0Android" >/dev/null 2>&1 &
}
export -f do_dl_payload_launcher
...
Community resources, tutorials, downloads and online help.\n" \
        ...
        --field="  <small><i>bzImage, initramfs, tutorials.</i></small>":LBL "" \
        \
        --field="":LBL "" \
        ...
        \
        --field="":LBL "" \
        --field="<b>— Debarshi(Deb).S — Payload-launcher —</b>":LBL "" \
        --field="  🐙 Open Payload-launcher (GitHub)":BTN 'bash -c "do_open_url_payload_launcher"' \
        --field="  ⬇  Download v2.0Android release":BTN 'bash -c "do_dl_payload_launcher"' \
        --field="  <small><i>Android payload launcher for PS4 by Debarshi(Deb).S.</i></small>":LBL "" \
        \
        --field="":LBL "" \
        --field="<b>— Ps4-Linux-Warez (triki1) —</b>":LBL "" \
        ...
        --field="  <small><tt>https://github.com/DEBARSHI-SARKAR/Payload-launcher</tt></small>":LBL "" \
        --field="  <small><tt>http://hostps4.free.fr/Ps4-Linux-Warez/index.html</tt></small>":LBL "" \
        \
        "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" \
        >/dev/null &
...
do_warez_dl_manager() {
    ...
    # PIDs des curls distros séparés (ne pas faire wait nu — bloquerait YAD)
    local distro_pids=()
...
class TblParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        elif tag in ('td','th') and self._r is not None: self._c=''; self._href=''
    ...
    def handle_endtag(self, tag):
        if tag == 'table' and self._t is not None:
            ...
            self._r.append((re.sub(r'\s+',' ',self._c).strip(), self._href)); self._c=None
    def handle_data(self, data):
...
class LinkParser(HTMLParser):
    def __init__(self):
        super().__init__()
        self.links=[]; self._t=None; self._h=''
    def handle_starttag(self, tag, attrs):
        if tag=='a': self._t=''; self._h=dict(attrs).get('href','')
    def handle_endtag(self, tag):
        if tag=='a' and self._t is not None:
            t=re.sub(r'\s+',' ',self._t).strip()
            if self._h: self.links.append((t,self._h))
            self._t=None
    def handle_data(self,data):
        if self._t is not None: self._t+=data
...
if html:
    ...
    for tbl in p.tables:
        for row in tbl:
            cols=[c[0] for c in row]; hrefs=[c[1] for c in row]
            if any(h.lower() in ('auteurs','author','fichier','file') for h in cols): continue
...
if html:
    ...
    for tbl in p.tables:
        for row in tbl:
            cols=[c[0] for c in row]; hrefs=[c[1] for c in row]
            if any(h.lower() in ('auteurs','author','fichier','file','version') for h in cols): continue
            url=next((h for h in hrefs if h and ('mega.nz' in h or h.startswith('http'))),'')
            if not url: continue
            author=cols[0] if cols else 'Unknown'
...
for fname_pg,label in distro_names.items():
    ...
    for text,href in lp.links:
        if href and ('mega.nz' in href or '1fichier.com' in href):
            results.append(('📦','triki1',label,'distro',svc(href),href)); break
...
https://1fichier.com/?at7ccs9f5rb9spzkm8h9|distro|Xubuntu PS4 [19,7 Go]	triki1	Xubuntu PS4  [19,7 Go]	distro	1fichier"
    ...
    while true; do
        ...
        sel=$(yad --center --borders=10 \
            ...
            --list \
            --text="$(printf '<big><b><span foreground="#4FC3F7">⬇  Ps4-Linux-Warez — Downloads</span></b></big>\n'
                    '<span foreground="#81D4FA"><b>hostps4.free.fr</b> · bzImages · Initramfs Vvsx87 · Distributions triki1</span>\n'
                    '<small>🐧 Kernels → <tt>%s</tt>\n'
                    '💾 Initramfs → <tt>%s</tt>\n'
                    '📦 Distros → <tt>%s</tt>\n'
                    'Mega.nz → mega-get · 1fichier → browser</small>'
                    "${DL_KERNELS_DIR//&/&amp;}" \
                    "${DL_INITRAMFS_DIR//&/&amp;}" \
                    "${DL_DISTROS_DIR//&/&amp;}")" \
            --column="PAYLOAD:HD" \
...
<span foreground='#FFF9C4'><i>PS4-TOOLS — made with passion for the PS4 Linux community</i></span>\n" \
        \
        --field="":LBL "" \
        ...
        \
        --field="":LBL "" \
        --field="<b><span foreground='#FFD54F'>— PS4-TOOLS —</span></b>":LBL "" \
        --field="  <small><b>Author:</b> Triki1.</small>":LBL "" \
        --field="  <small><b>Goal:</b> simplifying PS4 Linux for everyone, from beginner to kernel developer.</small>":LBL "" \
        --field="  <small><i>This project is a tribute to all those who gave their time to open the PlayStation 4 to the free world.</i></small>":LBL "" \
        --field="  🐧 Long live Linux on PS4! 🎮":LBL "" \
        \
        --field="":LBL "" \
        --field="  🌐 PS4-LINUX-WAREZ — Tutorial":BTN 'bash -c "xdg-open http://hostps4.free.fr/Ps4-Linux-Warez/tutoriels.html &"' \
...
<span foreground='${C_SECTION}'>Community  ·  Ps4-Linux-Warez  ·  DionKill  ·  Documentation</span>\n" \
        ...
        \
        --field="  🎮 <b>triki1</b> — Ps4-Linux-Warez: All in One (bzImages, Initramfs, Distros)":BTN \
        ...
        \
        --field="  🐙 <b>Debarshi(Deb).S</b> — Payload-launcher (Android)":BTN \
            'bash -c "do_open_url_payload_launcher"' \
        --field="  <small><tt>https://github.com/DEBARSHI-SARKAR/Payload-launcher</tt></small>":LBL "" \
        --field="  ⬇  Download v2.0Android release":BTN \
            'bash -c "do_dl_payload_launcher"' \
        \
        --field="  📖 <b>Dionkill</b> — PS4 Linux Tutorial":BTN \
...
yad --notebook \
    ...
    --title="PS4-TOOLS v6.0 Beta" \
    --width=1000 --height=740 \
    --image="$LOGO" \
    --image-on-top \
    --text="<span size='x-large'><b><span foreground='${C_TITRE}'>PS4-TOOLS</span></b></span>  <small><span foreground='${C_SECTION}'>v6.0 — by Triki1</span></small>
